...
function draw(){var pal=MAPS[S.mi].pal;ctx.fillStyle=pal.ceil;ctx.fillRect(0,0,W,H/2);ctx.fillStyle=pal.floor;ctx.fillRect(0,H/2,W,H/2);var fov=S.aim?0.85:1.28,rays=Math.min(140,W/3|0),zbuf=[],c;for(c=0;c<rays;c++){var ang=S.yaw+(c/rays-0.5)*fov,ca=Math.cos(ang),sa=Math.sin(ang),dist=0,hit=0;while(dist<16){dist+=0.08;hit=cell(S.x+ca*dist,S.y+sa*dist);if(hit)break;}var z=dist*Math.cos(ang-S.yaw);zbuf[c]=z;var colH=Math.min(H,(H*1.05)/Math.max(0.2,z)),shade=Math.max(0.25,1-dist/16);ctx.fillStyle=hit===2?shadeHex(pal.wall2,shade):shadeHex(pal.wall,shade);ctx.fillRect(c*(W/rays),(H-colH)/2-S.pitch*90+S.z*18,Math.ceil(W/rays)+1,colH);}var list=[];for(var i=0;i<foes.length;i++)if(foes[i].a)list.push(foes[i]);if(S.live)list.push({x:MAPS[S.mi].ext[0],y:MAPS[S.mi].ext[1],ext:1,a:1});list.sort(function(a,b){return Math.hypot(S.x-b.x,S.y-b.y)-Math.hypot(S.x-a.x,S.y-a.y)});for(i=0;i<list.length;i++){var f=list[i],dx=f.x-S.x,dy=f.y-S.y,dist=Math.hypot(dx,dy);if(dist<0.2)continue;var ang=Math.atan2(dy,dx)-S.yaw;while(ang>Math.PI)ang-=Math.PI*2;while(ang<-Math.PI)ang+=Math.PI*2;if(Math.abs(ang)>fov)continue;var sx=(0.5+ang/fov)*W,col=((sx/W)*rays)|0;if(col>=0&&col<zbuf.length&&zbuf[col]+0.15<dist)continue;var sh=Math.min(H*0.55,(H*0.55)/dist),bob=S.z*18-S.pitch*90;if(f.ext){ctx.fillStyle="#7dffb3";ctx.globalAlpha=0.7;ctx.fillRect(sx-sh*0.2,H/2+sh*0.3+bob,sh*0.4,8);ctx.globalAlpha=1;}else{var mid=H/2+bob, w=Math.max(6,sh*0.18), hgt=Math.max(14,sh*0.72);var top=mid-hgt*0.55;ctx.fillStyle="#2c241c";ctx.fillRect(sx-w*0.35, top+hgt*0.55, w*0.28, hgt*0.45);ctx.fillRect(sx+w*0.08, top+hgt*0.55, w*0.28, hgt*0.45);ctx.fillStyle="#3d4a3a";ctx.fillRect(sx-w*0.42, top+hgt*0.28, w*0.84, hgt*0.32);ctx.fillStyle="#c4a07a";ctx.beginPath();ctx.arc(sx, top+hgt*0.18, Math.max(3,w*0.28), 0, Math.PI*2);ctx.fill();ctx.fillStyle="#1c2018";ctx.fillRect(sx-w*0.32, top+hgt*0.06, w*0.64, hgt*0.12);ctx.fillStyle="#0e120c";ctx.fillRect(sx-w*0.22, top+hgt*0.16, w*0.44, hgt*0.06);ctx.fillStyle="#1a1c16";ctx.fillRect(sx+w*0.2, top+hgt*0.34, w*0.9, hgt*0.07);}}ctx.fillStyle="#161814";var gx=S.aim?W*0.48:W*0.62,gy=(S.aim?H*0.62:H*0.72)+(IN.fire?6:0);ctx.fillRect(gx,gy,18,70);ctx.fillRect(gx+6,gy-40,8,46);ctx.fillStyle="#6a5a3a";ctx.fillRect(gx-8,gy+28,16,36);}
